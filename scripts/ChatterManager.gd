...
func _on_damaged_event(data: Dictionary) -> void:
	...
	if not target.chatter:
		return

	var line: String = target.chatter.get_damaged_line(DAMAGED, amount, ctx)
...
func _on_attacking_event(data: Dictionary) -> void:
	...
	if not source.chatter:
		return

	var line: String = source.chatter.get_attacking_line(ATTACKING, source, targets)
