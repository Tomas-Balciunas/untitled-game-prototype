...
func game_load(data: Dictionary) -> void:
	...
	type      = data.get("type", 0) as Item.ItemType
...
static func from_dict(data: Dictionary) -> GearInstance:
	...
	match data.get("class", ""):
		...
		_:
			push_error("GearInstance.from_dict: unknown class '%s'" % data.get("class", ""))
			return null
	item.game_load(data)
	return item
